...
int alt_voice_extn_start_call(struct audio_device *adev)
{
    ...
    usecase_id = phone_type == 1 ? USECASE_VOICE_CALL : USECASE_VOICE2_CALL;
                                             ...
                                             struct voice_session **session)
{
    ...
    idx = phone_type == 1 ? VOICE_SESS_IDX : VOICE2_SESS_IDX;
    *session = &adev->voice.session[idx];
    return 0;
}

int alt_voice_extn_set_parameters(struct audio_device *adev __unused,
                                  struct str_parms *parms)
{
    ...
    if (ret >= 0) {
        ...
        if (property_get_int32(RADIO_PREFER_NETWORK_SLOT0) == property_get_int32(RADIO_PREFER_NETWORK_SLOT1))
                voice_slot = 0;
        if (strcmp(value, AUDIO_PARAMETER_VALUE_CP2)) {
                phone_type = voice_slot == 0 ? 1 : 0;
        } else {
                phone_type = voice_slot == 0 ? 0 : 1;
        }
        #elif SAMSUNG_DUAL_SIM
        phone_type = property_get_int32(
                strcmp(value, AUDIO_PARAMETER_VALUE_CP2) ?
                AUDIO_PROPERTY_SEC_VSID1 : AUDIO_PROPERTY_SEC_VSID2) + 1;
        #endif
        ALOGV("%s: phone_type: %d", __func__, phone_type);
    }
...
int alt_voice_extn_get_active_session_id(struct audio_device *adev __unused,
                                         uint32_t *session_id)
{
    *session_id = phone_type == 1 ? VOICE_VSID : VOICE2_VSID;
    return 0;
}
