...
#include "voice_extn/alt_voice_extn.h"
#include "platform.h"
                              ...
                              audio_usecase_t usecase_id)
{
    ...
    ret = alt_voice_extn_get_session_from_use_case(adev, usecase_id, &session);
    if (ret == 0) {
        return session;
    }
...
bool voice_is_call_state_active(struct audio_device *adev)
{
    ...
    ret = alt_voice_extn_is_call_state_active(adev, &call_state);
    if (ret == 0) {
        return call_state;
    }
...
uint32_t voice_get_active_session_id(struct audio_device *adev)
{
    ...
    ret = alt_voice_extn_get_active_session_id(adev, &session_id);
    if (ret == 0) {
        return session_id;
    }
...
int voice_start_call(struct audio_device *adev)
{
    ...
    ret = alt_voice_extn_start_call(adev);
    if (ret != -ENOSYS) {
...
int voice_stop_call(struct audio_device *adev)
{
    ...
    ret = alt_voice_extn_stop_call(adev);
    if (ret != -ENOSYS) {
        return 0;
    }
...
int voice_set_parameters(struct audio_device *adev, struct str_parms *parms)
{
    ...
    ret = alt_voice_extn_set_parameters(adev, parms);
    if (ret != 0 && ret != -ENOSYS)
        goto done;
