...
int amplifier_set_input_devices(void* adev, uint32_t devices) {
    amp.adev = (struct audio_device*)adev;
    if (amp.hw && amp.hw->set_input_devices) return amp.hw->set_input_devices(amp.hw, amp.adev, devices);
...
int amplifier_set_output_devices(void* adev, uint32_t devices) {
    amp.adev = (struct audio_device*)adev;
    if (amp.hw && amp.hw->set_output_devices) return amp.hw->set_output_devices(amp.hw, amp.adev, devices);
...
int amplifier_set_parameters(void* adev, struct str_parms* parms) {
    amp.adev = (struct audio_device*)adev;
    if (amp.hw && amp.hw->set_parameters) return amp.hw->set_parameters(amp.hw, amp.adev, parms);
