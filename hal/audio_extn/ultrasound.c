...
void us_manual_cal_load(void)
{
    ...
    if (!f) {
        ALOGE("%s: Cannot open calibration file: %s", __func__, ULTRASOUND_MANUAL_CALIBRATION_FILE);
        return;
    }
    ...
    fread(buff, 1, sizeof(buff), f);
    fclose(f);
    us_set_manual_cal(atoi(buff) + 84);
}
...
int us_init(struct audio_device *adev)
{
    ...
    memset(us, 0, sizeof(struct ultrasound_device));
    us->adev = adev;
...
int stop_us(void)
{
    ...
    ALOGD("%s: exit: status(%d)", __func__, 0);
...
int us_start(void)
{
    unsigned int rx_device_id, tx_device_id;
    struct audio_usecase *rx_usecase, *tx_usecase;
    ...
    if ((rx_device_id >> 31) | (tx_device_id >> 31))
    {
    ...
    memset(rx_usecase, 0, sizeof(struct audio_usecase));
    ...
    us->rx_pcm = pcm_open(us->adev->snd_card, rx_device_id, 0, &pcm_config_us);
    ...
    memset(tx_usecase, 0, sizeof(struct audio_usecase));
    ...
    us->tx_pcm = pcm_open(us->adev->snd_card, tx_device_id, 0x10000000, &pcm_config_us);
