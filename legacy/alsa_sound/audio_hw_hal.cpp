...
static amplifier_module_t *_amp_module = NULL;
static amplifier_module_t * get_amplifier_module(void)
{
    int rc;
...
static int amplifier_init(void)
{
    amplifier_module_t *amp = get_amplifier_module();
    if (amp && amp->init)
        return amp->init(amp);
...
static int amplifier_shutdown(void)
{
    amplifier_module_t *amp = get_amplifier_module();
    if (amp && amp->shutdown)
        return amp->shutdown(amp);
