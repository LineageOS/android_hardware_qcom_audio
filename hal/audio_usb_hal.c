...
static size_t split_audio_haptic_data(struct stream_out *out,
                 const void *buffer, size_t bytes_to_write)
{
    ...
    size_t bytes_per_sample = audio_bytes_per_sample(audio_format);
    size_t frame_size = channel_count * bytes_per_sample;
    size_t frame_count = bytes_to_write / frame_size;

    int    haptic_channel_count =  audio_channel_count_from_out_mask(out->hal_channel_mask & AUDIO_CHANNEL_HAPTIC_ALL);
    ...
    if (out->haptic_buffer == NULL) {
        out->haptic_buffer = (uint8_t *)calloc(1, total_haptic_buffer_size);
        if(out->haptic_buffer == NULL) {
    ...
    uint8_t *haptic_buffer  = out->haptic_buffer;
    ...
    out->haptic_buffer_size = frame_count * haptic_frame_size;
...
static ssize_t out_write(struct audio_stream_out *stream, const void* buffer, size_t bytes)
{
    ...
    if (out->hal_channel_mask & AUDIO_CHANNEL_HAPTIC_ALL)
        contracted_audio_bytes = split_audio_haptic_data(out, buffer, bytes);
    ...
    list_for_each(node, &out->alsa_devices) {
        ...
        int num_req_channels = 0;
        void *write_buff = buffer;
        if (out->hal_channel_mask & AUDIO_CHANNEL_HAPTIC_ALL) {
            size_t total_channel_count = audio_channel_count_from_out_mask(out->hal_channel_mask);
          ...
          num_req_channels = out->hal_channel_count;
          num_write_buff_bytes = bytes;
        }
                                   ...
                                   const char *address /*__unused*/)
{
    ...
    out->haptic_buffer = NULL;
    out->haptic_buffer_size = 0;
